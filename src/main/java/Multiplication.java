...
//				}
				...
				List<String> userRatings = RawDataMap.movieToUserRating.get(key.toString());
				...
				for (Map.Entry<String, Double> entry : relationMap.entrySet()) {
					...
					for (String userRating : userRatings) {
						String[] userRatingSplit = userRating.split(":");
						String user = userRatingSplit[0];
						Double rating = Double.parseDouble(userRatingSplit[1]);
		...
		public static void main(String[] args) throws Exception {
			...
			//MultipleInputs
			//		.addInputPath(job, new Path(args[0]), TextInputFormat.class, CooccurrenceMapper.class);
